...
const TodaySalesList = () => {
  ...
  const getActiveOrders = async () => {
    ...
    try {
      const response = await getRequest(`kot/today-sales-list/?all=false`);
...
const location = useLocation()
    ...
    const hasCCntrPrivilege =  hasPrivilege(user.privileges, userPrivileges.cashCounter);
    return (
      <div
        key={order.Code}
        className="
          ...
          <div className="flex items-center space-x-4">
            { (
             ...
             duration-200 shadow-sm"
               ...
               {isBillPrintFirst && order.CashCounterStatus==1  && hasCCntrPrivilege && <button
